...
{
    public partial class Customer_legers_form : Form
    {
        ...
        private void Customer_legers_form_Load(object sender, EventArgs e)
        {
            try
            {
                refresh();
        ...
        private void DisplayReportInReportViewer(ReportViewer viewer, string condition)
        {
            try
            {
                customer_ledger_ds report = new customer_ledger_ds();
                GetSetData.query = @"SELECT  pos_customer_transactions.date, pos_customer_transactions.billNo, pos_customer_transactions.status, pos_customer_transactions.net_amount, pos_customer_transactions.debit, pos_customer_transactions.credits, pos_customer_transactions.pCredits,
                                    ABS(pos_customers.opening_balance + SUM(ISNULL(net_amount, 0) - ISNULL(debit, 0)) OVER (ORDER BY pos_customer_transactions.date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)) as Balance, pos_customers.full_name, pos_customers.cus_code, pos_customers.opening_balance, pos_customer_lastCredits.lastCredits  
                ...
                {
                    GetSetData.query += " WHERE pos_customers.full_name = '" + customer_name_text.Text + "' and pos_customers.cus_code = '" + customer_code_text.Text + "' ORDER BY pos_customer_transactions.date asc;";
                }
        ...
        private void view_button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DisplayReportInReportViewer(this.viewer_all_transaction, "nill");
                }
            }
            catch (Exception es)
            {
