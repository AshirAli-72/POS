...
namespace Demands_info.forms
{
    public partial class formAddNewDemand : Form
    {
        ...
        {
            GetSetData.query = "select product_id from pos_products where  prod_name = '" + prod_name_text.Text + "' and barcode = '" + txt_barcode.Text + "';";
            GetSetData.Ids = data.Select_ID_for_Foreign_Key_from_db_for_Insertion(GetSetData.query);

            string purchase_price_db = data.UserPermissions("pur_price", "pos_stock_details", "prod_id", GetSetData.Ids.ToString());
            txt_pur_price.Text = purchase_price_db;

            string sale_price_db = data.UserPermissions("sale_price", "pos_stock_details", "prod_id", GetSetData.Ids.ToString());
            txt_sale_price.Text = sale_price_db;

            string paking_db = data.UserPermissions("pkg", "pos_stock_details", "prod_id", GetSetData.Ids.ToString());

            string tab_pieces_db = data.UserPermissions("tab_pieces", "pos_stock_details", "prod_id", GetSetData.Ids.ToString());
            txt_prod_state.Text = data.UserPermissions("prod_state", "pos_products", "product_id", GetSetData.Ids.ToString());
        ...
        private bool add_records_grid_view()
        {
            ...
            try
            {
                ...
                int category_id_db = data.UserPermissionsIds("category_id", "pos_category", "product_id", prod_id_db.ToString());
                int color_id_db = data.UserPermissionsIds("color_id", "pos_color", "product_id", prod_id_db.ToString());
                // ******************************************************************
