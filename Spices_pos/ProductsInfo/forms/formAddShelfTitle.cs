...
{
    public partial class formAddShelfTitle : Form
    {
        ...
        {
            GetSetData.SaveLogHistoryDetails("New Shelf Title Form", "Exit...", role_id);
            this.Close();
        }
        ...
        private void insert_records()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"insert into pos_shelfItems (title) values ('" + title_text.Text.ToString() + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    GetSetData.SaveLogHistoryDetails("New Shelf Title Form", "Saving Shelf [" + title_text.Text + "]", role_id); 
        ...
        private bool DeleteItems()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = "delete from pos_shelfItems where title = '" + title_text.Text + "';";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    GetSetData.SaveLogHistoryDetails("New Shelf Title Form", "Deleting Shelf [" + title_text.Text + "]", role_id); 
                }
                else
                {
        ...
        private void FillComboBoxWithItems()
        {
            try
            {
                ...
                GetSetData.FillComboBoxWithValues("select * from pos_shelfItems;", "title", title_text);
            }
            catch (Exception es)
            {
                error.errorMessage(es.Message);
        ...
        private void formAddShelfTitle_Load(object sender, EventArgs e)
        {
            try
            {
