    ...
    public partial class new_sub_category : Form
    {
        ...
        private void insert_records()
        {
            try
            {
                if (title_text.Text != "")
                {
                    //string quer_get_category_id_db = @"select category_id from Category where title = '" + cat_title.ToString() + "';";
                    //int category_id_db = data.Select_ID_for_Foreign_Key_from_db_for_Insertion(quer_get_category_id_db);

                    GetSetData.query = @"insert into pos_subcategory(title) values ('" + title_text.Text.ToUpper() + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    //GetSetData.SaveLogHistoryDetails("Sub Category Title Form", "Saving Sub Category [" + title_text.Text + "]", role_id); 
        ...
        private bool DeleteItems()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = "delete from pos_subcategory where title = '" + title_text.Text + "';";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    //GetSetData.SaveLogHistoryDetails("Sub Category Title Form", "Deleting Sub Category [" + title_text.Text + "]", role_id); 
                }
                else
                {
        ...
        {
            GetSetData.FillComboBoxUsingProcedures(title_text, "fillComboBoxSubCategory", "title");
        }
        ...
        {
            GetSetData.SaveLogHistoryDetails("Sub Category Title Form", "Exit...", role_id);
            this.Close();
        }
        ...
        private void new_sub_category_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                ClassDefaultValuesSetInDB.InsertValuesInTableSubCategory();
                FillComboBoxWithItems();

                title_text.Text = sub_category;
            }
            catch (Exception es)
            {
                error.errorMessage(es.Message);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"update pos_subcategory set title = '" + title_text.Text.ToUpper() + "' where (title = '" + sub_category + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);
