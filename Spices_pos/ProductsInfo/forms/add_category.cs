...
namespace Products_info.forms
{
    public partial class add_category : Form
    {
        ...
        {
            GetSetData.SaveLogHistoryDetails("Category Title Form", "Exit...", role_id);
            this.Close();
        }
        ...
        private void insert_records()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"insert into pos_category (title) values ('" + title_text.Text.ToUpper() + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    //GetSetData.SaveLogHistoryDetails("Category Title Form", "Saving category [" + title_text.Text + "]", role_id); 
        ...
        private bool DeleteItems()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = "delete from pos_category where title = '" + title_text.Text + "';";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    //GetSetData.SaveLogHistoryDetails("Category Title Form", "Deleting category [" + title_text.Text + "]", role_id); 
                }
                else
                {
        ...
        {
            GetSetData.FillComboBoxUsingProcedures(title_text, "fillComboBoxCategory", "title");
        }
        ...
        private void add_category_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                ClassDefaultValuesSetInDB.InsertValuesInTableCategory();
                FillComboBoxWithItems();

                title_text.Text = TextData.category;
            }
            catch (Exception es)
            {
                error.errorMessage(es.Message);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"update pos_category set title = '" + title_text.Text.ToUpper() + "' where (title = '" + TextData.category + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);
