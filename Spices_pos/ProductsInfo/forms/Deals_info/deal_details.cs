...
{
    public partial class deal_details : Form
    {
        ...
        private void FillGridViewUsingPagination(string condition)
        {
            try
            {
                GetSetData.query = @"select * from ViewDealDetails";

                if (condition == "search")
                {
                    GetSetData.query = GetSetData.query + " where ([Promotion ID] like '" + SearchByBox.Text + "%' or [Promotion Title] like '" + SearchByBox.Text + "%' or [Status] like '" + SearchByBox.Text + "%')";
                }
        ...
        {
            using (add_deal add_customer = new add_deal())
            {
                add_deal.role_id = role_id;
                add_deal.saveEnable = false;
        ...
        private bool fun_update_details()
        {
            try
            {
                add_deal.saveEnable = true;
                TextData.dealId = Convert.ToInt32(productDetailGridView.SelectedRows[0].Cells["Promotion ID"].Value.ToString());
                TextData.dealTitle = productDetailGridView.SelectedRows[0].Cells["Promotion Title"].Value.ToString();
        ...
        private bool fun_delete_products()
        {
            try
            {
                ...
                GetSetData.query = @"delete from pos_deal_items where (deal_id = '" + TextData.dealId.ToString() + "');";
                data.insertUpdateCreateOrDelete(GetSetData.query);
                //*****************************************************************************************

                GetSetData.query = @"delete from pos_deals where (deal_id = '" + TextData.dealId.ToString() + "');";
                data.insertUpdateCreateOrDelete(GetSetData.query);
                //*****************************************************************************************
        ...
        private void productDetailGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                TextData.dealId = Convert.ToInt32(productDetailGridView.SelectedRows[0].Cells["Promotion ID"].Value.ToString());
                GetSetData.FillDataGridView(productDetailGridView1, "ProcedureDealItemsList", TextData.dealId.ToString());
            }
            catch (Exception es)
            {
                error.errorMessage(es.Message);
