...
namespace Products_info.forms
{
    public partial class form_color : Form
    {
        ...
        {
            GetSetData.SaveLogHistoryDetails("Product Color Title Form", "Exit...", role_id);
            this.Close();
        }
        ...
        private void insert_records()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"insert into pos_color (title) values ('" + title_text.Text.ToUpper() + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    //GetSetData.SaveLogHistoryDetails("Product Color Title Form", "Saving Color [" + title_text.Text.ToUpper() + "]", role_id); 
        ...
        private bool DeleteItems()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = "delete from pos_color where title = '" + title_text.Text + "';";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    //GetSetData.SaveLogHistoryDetails("Product Color Title Form", "Deleting Color [" + title_text.Text + "]", role_id); 
                }
                else
                {
        ...
        {
             GetSetData.FillComboBoxUsingProcedures(title_text, "fillComboBoxColors", "title");
        }
        ...
        private void form_color_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                ClassDefaultValuesSetInDB.InsertValuesInTableColors();
                FillComboBoxWithItems();

                title_text.Text = colorValue;
            }
            catch (Exception es)
            {
                error.errorMessage(es.Message);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"update pos_color  set title = '" + title_text.Text.ToUpper() + "' where (title = '" + colorValue + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);
