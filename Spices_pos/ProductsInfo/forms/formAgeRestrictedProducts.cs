...
{
    public partial class formAgeRestrictedProducts : Form
    {
        ...
        private void FillGridViewUsingPagination(string condition)
        {
            try
            {
                GetSetData.query = "select top 200 * from ViewGroupedItems";
        ...
        private void formAgeRestrictedProducts_Load(object sender, EventArgs e)
        {
            try
            {
                //originalExStyle = -1;
                ...
                txtAgeLimit.Text = GetSetData.ProcedureGeneralSettings("ProcedureGeneralSettings", "customerAgeLimit");
        ...
        private void saved_gouped_items_db()
        {
            try
            {
                if (txtAgeLimit.Text != "")
                {
                    ...
                    foreach (DataGridViewRow item in ProductsDetailGridView.Rows)
                    {
                        ...
                        if (GetSetData.checkBoxSelected)
                        {
                            ...
                            GetSetData.query = "select group_id from pos_age_restricted_items where (prod_id = '" + product_id + "');";
                            GetSetData.fks = data.Select_ID_for_Foreign_Key_from_db_for_Insertion(GetSetData.query);

                            if (GetSetData.fks == 0)
                            {
                                GetSetData.query = "insert into pos_age_restricted_items values ('" + product_id + "' , '" + txtAgeLimit.Text + "');";
                ...
                {
                    error.errorMessage("Please enter the age limit first!");
                    error.ShowDialog();
                }
            }
            catch (Exception es)
            ...
            try
            {
                ...
                foreach (DataGridViewRow item in ProductsDetailGridView.Rows)
                {
                    ...
                    if (GetSetData.checkBoxSelected)
                    {
                        ...
                        GetSetData.query = "delete from pos_age_restricted_items where (prod_id = '" + product_id + "');";
                        data.insertUpdateCreateOrDelete(GetSetData.query);
