...
{
    public partial class form_expired_items : Form
    {
        ...
        private void FillGridViewUsingPagination(string condition)
        {
            try
            {
                GetSetData.query = "select * from ViewExpiredItems";
                
                if (condition == "search")
                {
                    GetSetData.query = GetSetData.query + " where ([Expired Date] like '%" + search_box.Text + "%' or [Product Name] like '%" + search_box.Text + "%' or [Barcode] like '%" + search_box.Text + "%' or [Category] like '%" + search_box.Text + "%' or [Brand] like '%" + search_box.Text + "%';";
                }
        ...
        private void form_expired_items_Load(object sender, EventArgs e)
        {
            //originalExStyle = -1;
            ...
            try
            {
                ...
                foreach (DataGridViewRow item in ProductsDetailGridView.Rows)
                {
                    ...
                    if (GetSetData.checkBoxSelected)
                    {
                        ...
                        GetSetData.query = "delete from pos_expired_items where (prod_id = '" + product_id + "') and (stock_id = '" + stock_id + "');";
                        data.insertUpdateCreateOrDelete(GetSetData.query);

                        //GetSetData.SaveLogHistoryDetails("Expired Items Details Form", "Remove [" + TextData.prod_name + "  " + TextData.barcode + "] from list", role_id);
                    }
                }
