...
{
    public partial class add_new_expense : Form
    {
        ...
        private void insert_records()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.Ids = data.UserPermissionsIds("exp_id", "pos_expenses", "title", title_text.Text);

                    if (GetSetData.Ids == 0)
                    {
                        GetSetData.query = @"insert into pos_expenses (title) values ('" + title_text.Text.ToString() + "');";
                        data.insertUpdateCreateOrDelete(GetSetData.query);

                        GetSetData.SaveLogHistoryDetails("Expenses Title Form", "Saving expense title [" + title_text.Text + "]", role_id);
        ...
        private bool DeleteItems()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = "delete from pos_expenses where title = '" + title_text.Text + "';";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    //GetSetData.SaveLogHistoryDetails("Expenses Title Form", "Deleting expense title [" + title_text.Text + "]", role_id);
                }
                else
                {
        ...
        {
            GetSetData.FillComboBoxUsingProcedures(title_text, "fillComboBoxExpenseTitle", "title");
        }
        ...
        private void add_new_expense_Load(object sender, EventArgs e)
        {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"update pos_expenses set title = '" + title_text.Text.ToUpper() + "' where (title = '" + title + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);
