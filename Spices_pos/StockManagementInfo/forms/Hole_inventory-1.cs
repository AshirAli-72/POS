    ...
    public partial class Hole_inventory : Form
    {
        ...
        public void fill_data_grid_view()
        {
            try
            {
                ...
                //string quer_to_get_data_from_Products_db = "select prod_name, prod_code, Category.title, Brand.brand_title, Sub_category.title, quantity, carton_qty, unit, prod_state, pur_price, sale_price, comments from Products, Category, Sub_category, Brand where Category.category_id = Products.cat_id and Sub_category.sub_cate_id = Products.sub_cat_id and Brand.brand_id = Products.brnd_id;";
                string quer_to_get_data_from_Products_db = "select prod_name, prod_code, Category.title, Brand.brand_title, Sub_category.title, quantity, carton_qty, unit, prod_state, pur_price, sale_price, comments from Products inner join Category on Products.cat_id = Category.category_id inner join Sub_category on Products.sub_cat_id = Sub_category.sub_cate_id inner join Brand on Products.brnd_id = Brand.brand_id;";

                data.Connect();
                data.adptr_ = new SqlDataAdapter(quer_to_get_data_from_Products_db, data.conn_);
                data.dset_ = new System.Data.DataSet();
                data.adptr_.Fill(data.dset_, "Category, Sub_category, Brand, Products");
                ProductsDetailGridView.DataSource = data.dset_.Tables["Category, Sub_category, Brand, Products"];
           }
            catch (Exception es)
            {
                error.errorMessage(es.Message);
        ...
        public void method_for_searching()
        {
            try
            {
                ...
                string quer_to_get_data_from_Products_db = @"select prod_name, prod_code, Category.title, Brand.brand_title, Sub_category.title, quantity, carton_qty, unit, prod_state, pur_price, sale_price, comments from Products inner join Category on Products.cat_id = Category.category_id inner join Sub_category on Products.sub_cat_id = Sub_category.sub_cate_id inner join Brand on Products.brnd_id = Brand.brand_id where prod_name like '" + search_box.Text + "%' or prod_code  like '" + search_box.Text + "%' or  Category.title like '" + search_box.Text + "%' or Sub_category.title like '" + search_box.Text + "%' or Brand.brand_title like '" + search_box.Text + "%';";
