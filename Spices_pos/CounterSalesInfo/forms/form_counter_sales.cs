...
namespace CounterSales_info.forms
{
    public partial class form_counter_sales : Form
    {
        ...
        private void CartDataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (CartDataGridView.Columns[e.ColumnIndex].Name == "Delete")
                {
                    deleteRowFromGridView();
                }
                //else if (CartDataGridView.Columns[e.ColumnIndex].Name == "add")
                //{
                //    AddN/*oteInGridView();*/
                //}
        ...
        private void deleteRowFromGridView()
        {
            try
            {
                ...
                //*****************************************************************************************
                int employee_id_db = GetSetData.ProcedureGetIntegerValues("ProcedureGetIntergerValues", "employee_id", "pos_employees", "full_name", TextData.employee);
                ...
                string product_id_db = CartDataGridView.SelectedRows[0].Cells[10].Value.ToString();
                string stock_id_db = CartDataGridView.SelectedRows[0].Cells[11].Value.ToString();

                TextData.prod_state = GetSetData.ProcedureGetStringValues("ProcedureGetStringValues", "prod_state", "pos_products", "product_id", product_id_db);
                TextData.pkg = GetSetData.ProcedureGetNumericValues("ProcedureGetNumericValues", "pkg", "pos_stock_details", "stock_id", stock_id_db);
                double pur_price_db = GetSetData.ProcedureGetNumericValues("ProcedureGetNumericValues", "pur_price", "pos_stock_details", "stock_id", stock_id_db);
                double wholeSalePrice_db = GetSetData.ProcedureGetNumericValues("ProcedureGetNumericValues", "whole_sale_price", "pos_stock_details", "stock_id", stock_id_db);
                ...
                {
                    TextData.quantity = double.Parse(CartDataGridView.SelectedRows[0].Cells[1].Value.ToString());
                    TextData.full_pkg = TextData.quantity / TextData.pkg;
                }

                TextData.total_pur_price = pur_price_db * double.Parse(CartDataGridView.SelectedRows[0].Cells[1].Value.ToString());
                TextData.wholeSale = wholeSalePrice_db * double.Parse(CartDataGridView.SelectedRows[0].Cells[1].Value.ToString());

                GetSetData.query = @"insert into pos_void_orders values ('" + TextData.dates + "' , '" + CartDataGridView.SelectedRows[0].Cells[1].Value.ToString() + "'  , '" + TextData.pkg.ToString() + "' , '" + TextData.full_pkg.ToString() + "' , '" + CartDataGridView.SelectedRows[0].Cells[2].Value.ToString() + "', '" + TextData.total_pur_price.ToString() + "','" + CartDataGridView.SelectedRows[0].Cells[4].Value.ToString() + "', '" + CartDataGridView.SelectedRows[0].Cells[3].Value.ToString() + "' , '" + TextData.wholeSale.ToString() + "', '" + CartDataGridView.SelectedRows[0].Cells[8].Value.ToString() + "', '" + product_id_db.ToString() + "', '" + employee_id_db.ToString() + "', '" + customer_id_db.ToString() + "', '" + DateTime.Now.ToShortTimeString() +"');";
                data.insertUpdateCreateOrDelete(GetSetData.query);
                // **************************************************************************************



                GetSetData.Ids = CartDataGridView.CurrentCell.RowIndex;
                CartDataGridView.Rows.RemoveAt(GetSetData.Ids);
        ...
        private bool fun_add_records_from_menu_deals_gridview(string prod_name, string barcode,  string product_id, string stock_id, double quantity_para,  double deal_price)
        {
            try
            {
                TextData.prod_price = "0";
                ...
                //GetSetData.fks = data.UserPermissionsIds("product_id", "pos_products", "prod_name", prod_name);

                //GetSetData.query = @"select product_id from pos_products where (prod_name = '" + prod_name + "') and (barcode = '" + barcode.ToString() + "');";
                //GetSetData.fks = data.Select_ID_for_Foreign_Key_from_db_for_Insertion(GetSetData.query);
                //int product_id = data.Select_ID_for_Foreign_Key_from_db_for_Insertion(GetSetData.query);
                ...
                double market_price = data.NumericValues("market_value", "pos_stock_details", "stock_id", stock_id);
                double tab_pieces_db = data.NumericValues("tab_pieces", "pos_stock_details", "stock_id", stock_id);
                double discount = data.NumericValues("discount", "pos_stock_details", "stock_id", stock_id);


                TextData.prod_state = data.UserPermissions("prod_state", "pos_products", "product_id", product_id.ToString());

                GetSetData.query = "select pos_category.title from pos_products inner join pos_category on pos_products.category_id = pos_category.category_id where (product_id = '" + product_id.ToString() + "');";
                TextData.category = data.SearchStringValuesFromDb(GetSetData.query);

                TextData.quantity = quantity_para;
                TextData.rate = deal_price;
                TextData.wholeSale = wholeSale;
                TextData.discount = discount;
                double discount_db = discount;
                TextData.MarketPrice = market_price;
                ...
                {
                    //TextData.barcode = data.UserPermissions("barcode", "pos_products", "prod_name", prod_name);
                    TextData.barcode = data.UserPermissions("item_barcode", "pos_stock_details", "stock_id", stock_id);
                    barcode = "nill";
                }
                else
                {
                ...
                string temp = "";
                string temp_barcode = "";
                string temp_stock_id = "";
                string chkImeiNo = "";
                double checkQuantity = 0;

                for (int i = 0; i < CartDataGridView.Rows.Count; i++)
                {
                    temp = CartDataGridView.Rows[i].Cells[0].Value.ToString();
                    temp_barcode = CartDataGridView.Rows[i].Cells[8].Value.ToString();
                    chkImeiNo = CartDataGridView.Rows[i].Cells[8].Value.ToString();

                    if ((TextData.prod_name == temp) && (barcode == chkImeiNo))
                    {
                        temp_stock_id = stock_id;
                        temp = TextData.prod_name;
                        temp_barcode = barcode;
                        chkImeiNo = barcode;
                        string quantity = CartDataGridView.Rows[i].Cells[1].Value.ToString();
                        checkQuantity += double.Parse(quantity);
                        break;
                    }
                    else if ((TextData.prod_name == temp) && (barcode == temp_barcode))
                    {
                        temp_stock_id = stock_id;
                        temp = TextData.prod_name;
                        temp_barcode = barcode;
                ...
                if (TextData.prod_state == "carton" || TextData.prod_state == "bag" || TextData.prod_state == "Tablets")
                {
                    switch (txt_sale_option.Text)
                    {
                        case "Pack":
                            TextData.quantity = (TextData.quantity * pkg_db) * tab_pieces_db;
                            TextData.rate = TextData.quantity * deal_price;
                            TextData.wholeSale = TextData.quantity * wholeSale;
                            TextData.discount = TextData.quantity * discount;
                            //TextData.MarketPrice = TextData.quantity * market_price;
                            break;
                ...
                if ((temp != TextData.prod_name) && (chkImeiNo != barcode || barcode == "nill")) // Adding new item in the cart
                {
                    if (txt_status.Checked == false) // sale out items 
                    {
                        string isStockLimitSetToZero = data.UserPermissions("setStockLimitToZero", "pos_general_settings");

                        if (((TextData.quantity + checkQuantity) <= quantity_db) || (TextData.category == "Services") || (TextData.category == "MISCELLANEOUS") || (isStockLimitSetToZero == "No"))
                        {
                            int n = CartDataGridView.Rows.Add();
                            CartDataGridView.Rows[n].Cells[0].Value = TextData.prod_name;
                            CartDataGridView.Rows[n].Cells[1].Value = TextData.quantity.ToString();
                            CartDataGridView.Rows[n].Cells[2].Value = Math.Round(TextData.discount, 2).ToString();
                            ...
                            if (chkWholeSale.Checked == true)
                            {
                                ...
                                CartDataGridView.Rows[n].Cells[3].Value = Math.Round(TextData.MarketPrice, 2).ToString();

                                // *************************************

                                //TextData.wholeSale -= TextData.discount;
                                TextData.wholeSale = (TextData.wholeSale + TextData.MarketPrice) - TextData.discount;

                                CartDataGridView.Rows[n].Cells[4].Value = Math.Round(TextData.wholeSale, 2).ToString();

                                // *************************************

                                TextData.total_sales_price = TextData.quantity * wholeSale;
                            }
                            else
                            {
                                // calculating Tax
                                ...
                                CartDataGridView.Rows[n].Cells[4].Value = Math.Round(TextData.rate, 2).ToString();

                                // *************************************

                                TextData.total_sales_price = TextData.quantity * deal_price;
                            }

                            CartDataGridView.Rows[n].Cells[5].Value = quantity_db.ToString();
                            CartDataGridView.Rows[n].Cells[8].Value = barcode;
                            CartDataGridView.Rows[n].Cells[10].Value = product_id;
                            CartDataGridView.Rows[n].Cells[11].Value = stock_id;

                            // ********************************************************************************
                            TextData.no_of_items++;
                            txt_total_items.Text = TextData.no_of_items.ToString();

                            //TextData.total_taxation = double.Parse(txtTaxation.Text) + TextData.MarketPrice;
                            //txtTaxation.Text = Math.Round(TextData.total_taxation, 2).ToString();


                            TextData.net_total = double.Parse(txt_sub_total.Text) + TextData.total_sales_price;
                            txt_sub_total.Text = Math.Round(TextData.net_total, 2).ToString();
                            ...
                            TextData.discount = TextData.quantity * discount;
                            TextData.totalDiscount = double.Parse(txt_discount.Text) + TextData.discount;
                            txt_discount.Text = Math.Round(TextData.totalDiscount, 2).ToString();
                            txt_total_discount.Text = Math.Round(TextData.totalDiscount, 2).ToString();

                            //TextData.TotalMarketPrice = TextData.quantity * market_price;
                            //TextData.MarketPrice = TextData.TotalMarketPrice - TextData.total_sales_price;

                            //if (TextData.MarketPrice > 0)
                            //{
                            //    TextData.MarketPrice = double.Parse(txt_tax.Text) + TextData.MarketPrice;
                            //    txt_tax.Text = Math.Round(TextData.MarketPrice, 2).ToString();
                            //}
                ...
                else if ((temp == TextData.prod_name) && (chkImeiNo != barcode) && (barcode != "nill")) // Adding new item in the cart
                {
                    if (txt_status.Checked == false) // sale out items 
                    {
                        string isStockLimitSetToZero = data.UserPermissions("setStockLimitToZero", "pos_general_settings");
                ...
                else  // updating, items already exist in the cart 
                {
                    ...
                    for (int i = 0; i < CartDataGridView.Rows.Count; i++)
                    {
                        double prod_price = double.Parse(CartDataGridView.Rows[i].Cells[2].Value.ToString()) + double.Parse(CartDataGridView.Rows[i].Cells[4].Value.ToString());
                        TextData.prod_price = prod_price.ToString();

                        double cart_taxation = double.Parse(CartDataGridView.Rows[i].Cells[3].Value.ToString());
                        TextData.cart_taxation = cart_taxation.ToString();
                        ...
                        if ((TextData.prod_name == temp) && (barcode == temp_barcode || barcode == "nill"))
                        {
                            string quantity = CartDataGridView.Rows[i].Cells[1].Value.ToString();
                            total_qty = TextData.quantity;
                            TextData.quantity += double.Parse(quantity);
                            ...
                            if (txt_status.Checked == false) // sale out items???
                            {
                                ...
                                if ((TextData.quantity <= quantity_db) || (TextData.category == "Services") || (TextData.category == "MISCELLANEOUS") || (isStockLimitSetToZero == "No"))
                                {
                                    ...
                                    //TextData.MarketPrice = TextData.quantity * market_price;
                                    market_price = double.Parse(quantity) * market_price;


                                    TotalSalePrice = double.Parse(CartDataGridView.Rows[i].Cells[4].Value.ToString()) + double.Parse(CartDataGridView.Rows[i].Cells[2].Value.ToString());
                                    discount = double.Parse(CartDataGridView.Rows[i].Cells[2].Value.ToString());
                                    market_price = double.Parse(CartDataGridView.Rows[i].Cells[3].Value.ToString());


                                    CartDataGridView.Rows[i].Cells[1].Value = TextData.quantity.ToString();
                                    CartDataGridView.Rows[i].Cells[2].Value = Math.Round(TextData.discount, 2).ToString();
                                    CartDataGridView.Rows[i].Cells[3].Value = Math.Round(TextData.MarketPrice, 2).ToString();
                                    ...
                                    if (chkWholeSale.Checked == true)
                                    {
                                        ...
                                        CartDataGridView.Rows[i].Cells[4].Value = Math.Round(TextData.wholeSale, 2).ToString();
                                    }
                                    else
                                    {
                                        TextData.rate = TextData.rate * TextData.quantity;
                                        ...
                                        CartDataGridView.Rows[i].Cells[4].Value = Math.Round(TextData.rate, 2).ToString();
                                    }
                                    CartDataGridView.Rows[i].Cells[5].Value = quantity_db.ToString();
                                    CartDataGridView.Rows[i].Cells[8].Value = barcode;
                                    CartDataGridView.Rows[i].Cells[10].Value = product_id;
                                    CartDataGridView.Rows[i].Cells[11].Value = stock_id;
                                    ...
                                    if (chkWholeSale.Checked == true)
                                    {
                                        ...
                                        TextData.net_taxation = ((TextData.total_sales_price * market_price) / 100);
                                    ...
                                    TextData.net_total = ((double.Parse(txt_sub_total.Text) + TextData.total_sales_price) - double.Parse(TextData.prod_price));
                                    txt_sub_total.Text = Math.Round(TextData.net_total, 2).ToString();


                                    //TextData.total_taxation = ((double.Parse(txtTaxation.Text) + TextData.net_taxation) - double.Parse(TextData.cart_taxation));
                                    //txtTaxation.Text = Math.Round(TextData.total_taxation, 2).ToString();
                                    ...
                                    TextData.discount = 0;
                                    TextData.totalDiscount = 0;
                                    TextData.discount = TextData.quantity * discount_db;
                                    TextData.totalDiscount = (double.Parse(txt_discount.Text) + TextData.discount) - discount;
                                    txt_discount.Text = Math.Round(TextData.totalDiscount, 2).ToString();
                                    txt_total_discount.Text = Math.Round(TextData.totalDiscount, 2).ToString();
                                    //*******************************************************************


                                    market_price = market_price - TotalSalePrice;
                                    TextData.MarketPrice = TextData.MarketPrice - TextData.total_sales_price;

                                    if (TextData.MarketPrice > 0)
                                    {
                                        TextData.MarketPrice = ((double.Parse(txt_tax.Text) + TextData.MarketPrice) - market_price);
                                        txt_tax.Text = Math.Round(TextData.MarketPrice, 2).ToString();
                                    }

                                    TextData.quantity = double.Parse(txt_total_qty.Text) + total_qty;
                                    txt_total_qty.Text = TextData.quantity.ToString();
                                    beep_sound();
                                }
                                else
                ...
                txt_total_qty.Text = totalQuantity.ToString();
                txt_tax.Text = totalSiscount.ToString();
                txtTaxation.Text = totalTaxation.ToString();
