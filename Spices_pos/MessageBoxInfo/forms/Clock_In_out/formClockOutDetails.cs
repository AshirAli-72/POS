...
{
    public partial class formClockOutDetails : Form
    {
        ...
        private void FillGridViewUsingPagination(string condition)
        {
            try
            {
                GetSetData.query = "select top 500 * from ViewCashOutDetails";
                ...
                {
                    GetSetData.query = "select top 500 * from ViewCashOutDetails where ([Date] between '" + FromDate.Text + "' and '" + ToDate.Text + "')";
                } 
                else if (condition == "EmployeeWise")
                {
                    GetSetData.query = "select top 500 * from ViewCashOutDetails where ([Date] between '" + FromDate.Text + "' and '" + ToDate.Text + "') and ([User] = '" + txtEmployeeName.Text +"')";
                }
        ...
        private void formClockOutDetails_Load(object sender, EventArgs e)
        {
            try
            {
                //originalExStyle = -1;
        ...
        {
            formAddTimeClockHistory.user_id = user_id;
            formAddTimeClockHistory.role_id = role_id;
            formAddTimeClockHistory.saveEnable = false;

            using (formAddTimeClockHistory add_customer = new formAddTimeClockHistory())
            {
        ...
        private bool fun_update_details()
        {
            try
            {
                ...
                //{
                formAddClockOut.saveEnable = true;
                ...
                TextData.openingAmount = productDetailGridView.SelectedRows[0].Cells["Opening Cash"].Value.ToString();
                TextData.totalSales = productDetailGridView.SelectedRows[0].Cells["Total Sales"].Value.ToString();
                ...
                TextData.expectedAmount = productDetailGridView.SelectedRows[0].Cells["Expected Cash"].Value.ToString();
        ...
        private bool fun_delete_products()
        {
            try
            {
               ...
               TextData.clockIn_id = data.UserPermissions("clock_in_id", "pos_clock_out", "id", TextData.clockOut_id);
                ...
                //========================================================
                GetSetData.query = @"update pos_clock_in set status  = '0' where ( id = '" + TextData.clockIn_id + "');";
        ...
        private void formClockOutDetails_KeyDown(object sender, KeyEventArgs e)
        {
