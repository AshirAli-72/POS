...
{
    public partial class formCharityDetails : Form
    {
        ...
        private void system_user_permissions()
        {
            try
            {
                ...
                formCharityPayment.role_id = role_id;
        ...
        private void FillGridViewUsingPagination(string condition)
        {
            try
            {
                GetSetData.query = "select * from ViewCharityDetails";

                if (condition == "search")
                {
                    GetSetData.query = GetSetData.query + " where ([Date] like '" + SearchByBox.Text + "%' or [Full Name] like '" + SearchByBox.Text + "%') or [Father Name] like '" + SearchByBox.Text + "%') or [Mobile No] like '" + SearchByBox.Text + "%');";
                }
        ...
        private void formCharityDetails_Load(object sender, EventArgs e)
        {
            try
            {
                originalExStyle = -1;
        ...
        {
            using (formCharityPayment add_customer = new formCharityPayment())
            {
                //GetSetData.SaveLogHistoryDetails("Charity payment Details Form", "Add new charity payment button click...", role_id);
                formCharityPayment.saveEnable = false;
        ...
        private bool fun_update_details()
        {
            try
            {
                ...
                //{
                formCharityPayment.saveEnable = true;
                TextData.fromDate = productDetailGridView.SelectedRows[0].Cells["From Date"].Value.ToString();
                TextData.toDate = productDetailGridView.SelectedRows[0].Cells["To Date"].Value.ToString();
                TextData.date = productDetailGridView.SelectedRows[0].Cells["Payment Date"].Value.ToString();
                TextData.time = productDetailGridView.SelectedRows[0].Cells["Time"].Value.ToString();
                TextData.fullName = productDetailGridView.SelectedRows[0].Cells["Full Name"].Value.ToString();
                TextData.fatherName = productDetailGridView.SelectedRows[0].Cells["Father Name"].Value.ToString();
                TextData.mobileNo = productDetailGridView.SelectedRows[0].Cells["Mobile No"].Value.ToString();
                TextData.editAmount = productDetailGridView.SelectedRows[0].Cells["Payment"].Value.ToString();
                TextData.references = productDetailGridView.SelectedRows[0].Cells["References"].Value.ToString();
                TextData.remarks = productDetailGridView.SelectedRows[0].Cells["Note"].Value.ToString();
                TextData.editLessAmount = productDetailGridView.SelectedRows[0].Cells["Less Amount"].Value.ToString();
                TextData.editProfit = productDetailGridView.SelectedRows[0].Cells["Profit"].Value.ToString();
                TextData.editBalance = productDetailGridView.SelectedRows[0].Cells["Balance"].Value.ToString();

                using (formCharityPayment add_customer = new formCharityPayment())
                {
                    //GetSetData.SaveLogHistoryDetails("Charity payment Details Form", "Updating Charity payment [" + TextData.date + "  " + TextData.time + "] details (Modify button click...)", role_id);
                    add_customer.ShowDialog();
                }
        ...
        private bool fun_delete_products()
        {
            try
            {
                ...
                if (GetSetData.Data == "Yes")
                {
                    ...
                    {
                        TextData.cashAmount = (double.Parse(capital) + previousPaidAmount).ToString();
                    }
                ...
                GetSetData.query = @"delete from pos_charityDetails where (paymentDate = '" + TextData.date + "') and (time = '" + TextData.time + "');";
                data.insertUpdateCreateOrDelete(GetSetData.query);
                //========================================================

                GetSetData.SaveLogHistoryDetails("Charity payment Details Form", "Deleting charity payment [" + TextData.date + "  " + TextData.time + "] details", role_id);
                return true;
            }
            catch (Exception es)
            {
        ...
        private void formCharityDetails_KeyDown(object sender, KeyEventArgs e)
        {
