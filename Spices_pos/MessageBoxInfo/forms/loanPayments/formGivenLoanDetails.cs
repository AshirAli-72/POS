...
using Datalayer;
using Message_box_info.LoanReports;
...
{
    public partial class formGivenLoanDetails : Form
    {
        ...
        private void system_user_permissions()
        {
            try
            {
                ...
                formLoansGiven.role_id = role_id;
        ...
        private void FillGridViewUsingPagination(string condition)
        {
            try
            {
                GetSetData.query = "select * from ViewLoanGivenDetails";

                if (condition == "search")
                {
                    GetSetData.query = GetSetData.query + " where ([Date] like '" + SearchByBox.Text + "%' or [Full Name] like '" + SearchByBox.Text + "%' or [Father Name] like '" + SearchByBox.Text + "%' or [Mobile No] like '" + SearchByBox.Text + "%'  or [Status] like '" + SearchByBox.Text + "%');";
                }
        ...
        private void formGivenLoanDetails_Load(object sender, EventArgs e)
        {
            try
            {
                originalExStyle = -1;
        ...
        {
            GetSetData.SaveLogHistoryDetails("Loan Given Details Form", "Exit...", role_id);
        ...
        {
            using (formLoansGiven add_customer = new formLoansGiven())
            {
                GetSetData.SaveLogHistoryDetails("Loan Given Details Form", "Add new Loan button click...", role_id);
                formLoansGiven.saveEnable = false;
        ...
        private bool fun_update_details()
        {
            try
            {
                ...
                //{
                formLoansGiven.saveEnable = true;
                ...
                TextData.editAmount = productDetailGridView.SelectedRows[0].Cells["Amount"].Value.ToString();
                ...
                using (formLoansGiven add_customer = new formLoansGiven())
                {
                    GetSetData.SaveLogHistoryDetails("Loan Given Details Form", "Updating loan [" + TextData.date + "  " + TextData.time + "  " + TextData.status + "] details (Modify button click...)", role_id);
                    add_customer.ShowDialog();
                }
        ...
        private bool fun_delete_products()
        {
            try
            {
                ...
                GetSetData.query = @"delete from pos_loanDetails where (date = '" + TextData.date + "') and (time = '" + TextData.time + "');";
                data.insertUpdateCreateOrDelete(GetSetData.query);
                //========================================================

                GetSetData.SaveLogHistoryDetails("Loan Given Details Form", "Deleting loan [" + TextData.date + "  " + TextData.time + "] details", role_id);
                return true;
            }
            catch (Exception es)
            {
        ...
        {
            formLoanDetailsReport report = new formLoanDetailsReport();
            report.ShowDialog();
        }

        private void formGivenLoanDetails_KeyDown(object sender, KeyEventArgs e)
        {
