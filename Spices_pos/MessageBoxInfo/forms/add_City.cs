    ...
    public partial class add_City : Form
    {
        ...
        private void insert_records()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"insert into pos_city (title) values ('" + title_text.Text.ToString() + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    GetSetData.SaveLogHistoryDetails("Add New City Title Form", "Saving city [" + title_text.Text + "]", role_id); 
        ...
        private bool DeleteItems()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = "delete from pos_city where title = '" + title_text.Text + "';";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    GetSetData.SaveLogHistoryDetails("Add New City Title Form", "Deleting city [" + title_text.Text + "]", role_id); 
                }
                else
                {
        ...
        {
            GetSetData.FillComboBoxUsingProcedures(title_text, "fillComboBoxCityNames", "title");
        }

        private void showbutton_Click(object sender, EventArgs e)
        {
            GetSetData.SaveLogHistoryDetails("Add New City Title Form", "Exit...", role_id); 
            this.Close();
        }
