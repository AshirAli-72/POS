...
{
    public partial class formNewBatchNo : Form
    {
        ...
        private void insert_records()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"insert into pos_batchNo (title) values ('" + title_text.Text.ToString() + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    GetSetData.SaveLogHistoryDetails("Add New Batch No Form", "Saving [" + title_text.Text + "]", role_id);
        ...
        private bool DeleteItems()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = "delete from pos_batchNo where title = '" + title_text.Text + "';";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    GetSetData.SaveLogHistoryDetails("Add New Batch No Form", "Deleting [" + title_text.Text + "]", role_id);
                }
                else
                {
        ...
        private void FillComboBoxWithItems()
        {
            try
            {
                ...
                GetSetData.FillComboBoxWithValues("select * from pos_batchNo;", "title", title_text);
            }
            catch (Exception es)
            {
                error.errorMessage(es.Message);
        ...
        {
            //GetSetData.SaveLogHistoryDetails("Add New Batch No Form", "Exit", role_id);
            this.Close();
        }
