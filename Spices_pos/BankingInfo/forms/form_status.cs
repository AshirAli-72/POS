    ...
    public partial class form_status : Form
    {
        ...
        private void insert_records()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"insert into pos_transaction_status (status_title) values ('" + title_text.Text.ToString() + "');";
        ...
        {
            GetSetData.FillComboBoxUsingProcedures(title_text, "fillComboBoxTransactionStatus", "status_title");
        }
        ...
        {
            //GetSetData.SaveLogHistoryDetails("Bank Transaction Status Form", "Exit...", role_id);
            this.Close();
        }
        ...
        private bool DeleteItems()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"delete from pos_transaction_status where status_title = '" + title_text.Text + "';";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    //GetSetData.SaveLogHistoryDetails("Bank Transaction Status Form", "Deleting item [" + title_text.Text + "]", role_id);
                }
                else
                {
        ...
        private void form_status_Load(object sender, EventArgs e)
        {
            ...
            ClassDefaultValuesSetInDB.InsertValuesInTablesBankTransactionStatus();
            RefreshFieldTransactionStatus();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"update pos_transaction_status set status_title = '" + title_text.Text.ToUpper() + "' where (status_title = '" + title + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);
