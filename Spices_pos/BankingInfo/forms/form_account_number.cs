...
{
    public partial class form_account_number : Form
    {
        ...
        {
            GetSetData.FillComboBoxUsingProcedures(title_text, "fillComboBoxBankAccountNo", "account_no");
        }
        ...
        private void insert_records()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"insert into pos_account_no (account_no) values ('" + title_text.Text.ToString() + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    GetSetData.SaveLogHistoryDetails("Bank Account Number Form", "Saving account # [" + title_text.Text + "]", role_id);
        ...
        private bool DeleteItems()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"delete from pos_account_no where account_no = '" + title_text.Text + "';";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    //GetSetData.SaveLogHistoryDetails("Bank Account Number Form", "Deleting account # [" + title_text.Text + "]", role_id);
                }
                else
                {
        ...
        {
            //GetSetData.SaveLogHistoryDetails("Bank Account Number Form", "Exit...", role_id);
            this.Close();
        }

        private void form_account_number_Load(object sender, EventArgs e)
        {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"update pos_account_no set account_no = '" + title_text.Text.ToUpper() + "' where (account_no = '" + title + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);
