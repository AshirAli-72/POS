...
{
    public partial class form_account_title : Form
    {
        ...
        {
            GetSetData.FillComboBoxUsingProcedures(title_text, "fillComboBoxAccountTitle", "account_title");
        }
        ...
        private void insert_records()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"insert into pos_bank_account (account_title) values ('" + title_text.Text.ToString() + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    GetSetData.SaveLogHistoryDetails("Bank Account Title Form", "Saving account title [" + title_text.Text + "]", role_id);
        ...
        private bool DeleteItems()
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"delete from pos_bank_account where account_title = '" + title_text.Text + "';";
                    data.insertUpdateCreateOrDelete(GetSetData.query);

                    //GetSetData.SaveLogHistoryDetails("Bank Account Title Form", "Deleting account title [" + title_text.Text + "]" , role_id);
                }
                else
                {
        ...
        {
            //GetSetData.SaveLogHistoryDetails("Bank Account Title Form", "Exit...", role_id);
            this.Close();
        }
        ...
        private void form_account_title_Load(object sender, EventArgs e)
        {
            GetSetData.addFormCopyrights(lblCopyrights); 
            ClassDefaultValuesSetInDB.InsertValuesInTablesBankAccountTitles();
            RefreshFieldAccountTitle();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (title_text.Text != "")
                {
                    GetSetData.query = @"update pos_bank_account set account_title = '" + title_text.Text.ToUpper() + "' where (account_title = '" + title + "');";
                    data.insertUpdateCreateOrDelete(GetSetData.query);
