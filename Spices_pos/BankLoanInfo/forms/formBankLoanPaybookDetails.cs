...
using Banks_Loan_info.BankLoanPaymentReports;
...
{
    public partial class formBankLoanPaybookDetails : Form
    {
        ...
        public formBankLoanPaybookDetails()
        {
            InitializeComponent();
        ...
        private void system_user_permissions()
        {
            try
            {
                formAddNewLoanPayment.role_id = role_id;
                //GetSetData.addFormCopyrights(lblCopyrights);
                pnl_print.Visible = bool.Parse(data.UserPermissions("bankLoanPaybook_details_print", "pos_tbl_authorities_button_controls3", role_id));
                pnl_delete.Visible = bool.Parse(data.UserPermissions("bankLoanPaybook_details_delete", "pos_tbl_authorities_button_controls3", role_id));
                pnl_add_new.Visible = bool.Parse(data.UserPermissions("bankLoanPaybook_details_new", "pos_tbl_authorities_button_controls3", role_id));
                pnl_modify.Visible = bool.Parse(data.UserPermissions("bankLoanPaybook_details_modify", "pos_tbl_authorities_button_controls3", role_id));
        ...
        private void FillGridViewUsingPagination(string condition)
        {
            try
            {
                GetSetData.query = @"SELECT format(date, 'dd/MMMM/yyyy') AS Date, time as [Time], format(paymentDate, 'dd/MMMM/yyyy') as [Payment Date], 
                                    ...
                                    FROM ViewBankLoanPaybookDetails";

                if (condition == "search")
                {
                    GetSetData.query = GetSetData.query + " where (date like '" + search_box.Text + "%' or paymentDate like '" + search_box.Text + "%' or bank_name like '" + search_box.Text + "%' or code like '" + search_box.Text + "%' or status_title like '" + search_box.Text + "%');";
                }
        ...
        private void formBankLoanPaybookDetails_Load(object sender, EventArgs e)
        {
            try
            {
                //originalExStyle = -1;
        ...
        {
            using (formAddNewLoanPayment add_customer = new formAddNewLoanPayment())
            {
                formAddNewLoanPayment.saveEnable = false;
                GetSetData.SaveLogHistoryDetails("Bank Loan Paybook Details Form", "Add new bank Loan payment button click...", role_id);
                add_customer.ShowDialog();
                //button_controls.AddNewLoanPaymentButtons();
            }
        }
        ...
        private bool fun_update_details()
        {
            try
            {
                GetSetData.query = "select bankLoanPaybook_details_modify from pos_tbl_authorities_button_controls3 where role_id = '" + role_id.ToString() + "';";
                GetSetData.Data = data.SearchStringValuesFromDb(GetSetData.query);

                if (bool.Parse(GetSetData.Data) == true)
                {
                    formAddNewLoanPayment.saveEnable = true;
                    ...
                    TextData.paymentDate = ProductsDetailGridView.SelectedRows[0].Cells["Payment Date"].Value.ToString();
                    ...
                    TextData.CashAmount = double.Parse(ProductsDetailGridView.SelectedRows[0].Cells["Payment"].Value.ToString());
                    TextData.credits = double.Parse(ProductsDetailGridView.SelectedRows[0].Cells["Payables"].Value.ToString());
                    ...
                    using (formAddNewLoanPayment add_customer = new formAddNewLoanPayment())
                    {
                        GetSetData.SaveLogHistoryDetails("Bank Loan Paybook Details Form", "Modify Loan Payment details [" + TextData.date + " " + TextData.time + "] ....", role_id);
                        add_customer.ShowDialog();
                    }
                }
        ...
        private bool fun_delete_products()
        {
            try
            {
                ...
                TextData.bank_title = ProductsDetailGridView.SelectedRows[0].Cells["Bank Title"].Value.ToString();
                TextData.bankCode = ProductsDetailGridView.SelectedRows[0].Cells["Code"].Value.ToString();
                TextData.totalAmount = double.Parse(ProductsDetailGridView.SelectedRows[0].Cells["Payment"].Value.ToString());

                GetSetData.query = @"select loanPaybook_id from pos_bankLoanPaybook where (date = '" + TextData.date + "') and (time = '" + TextData.time + "');";
                GetSetData.Ids = data.Select_ID_for_Foreign_Key_from_db_for_Insertion(GetSetData.query);

                GetSetData.query = @"select BankLoan_id from pos_bankLoansDetails where (bank_name = '" + TextData.date + "') and (code = '" + TextData.time + "');";
                GetSetData.fks = data.Select_ID_for_Foreign_Key_from_db_for_Insertion(GetSetData.query);
                //========================================================

                //TextData.lastCredits = data.NumericValues("last_balance", "pos_bankLoanPayables", "BankLoan_id", GetSetData.fks.ToString());

                //TextData.credits = TextData.lastCredits + TextData.amount;
                ...
                //GetSetData.query = @"update pos_bankLoanPayables set last_balance = '" + TextData.credits.ToString() + "' where BankLoan_id = '" + GetSetData.fks.ToString() + "';";
                //data.insertUpdateCreateOrDelete(GetSetData.query);
                ////*****************************************************************************************

                GetSetData.query = @"delete from pos_bankLoanPaybook where loanPaybook_id = '" + GetSetData.Ids.ToString() + "';";
                data.insertUpdateCreateOrDelete(GetSetData.query);
                //*****************************************************************************************
                ...
                GetSetData.SaveLogHistoryDetails("Bank  Loan Paybook Details Form", "Deleting [" + TextData.date + "  " + TextData.time + "] details...", role_id);
        ...
        {
            //GetSetData.SaveLogHistoryDetails("Bank  Loan Paybook Details Form", "Print bank loan Payment details button click...", role_id);
            formBankLoanPaymentsReport list = new formBankLoanPaymentsReport();
            list.ShowDialog();
        }

        private void formBankLoanPaybookDetails_KeyDown(object sender, KeyEventArgs e)
        {
